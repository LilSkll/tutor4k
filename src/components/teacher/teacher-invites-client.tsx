...
export function TeacherInvitesClient() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { invites?: TeacherInviteRow[]; error?: string };
      if (!res.ok) throw new Error(data.error || "fail");
      setInvites(data.invites ?? []);
    } catch {
      toast.error(t("teacher.invites.loadFail"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const create = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { invite?: TeacherInviteRow; error?: string };
      if (!res.ok) throw new Error(data.error || "fail");
  ...
  const close = async (id: string) => {
    try {
      const res = await fetch(`/api/teacher/invites?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">{t("teacher.invites.title")}</h2>
        <p className="mt-1 text-sm text-muted-foreground">{t("teacher.invites.subtitle")}</p>
      </div>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">{t("teacher.invites.createTitle")}</CardTitle>
        </CardHeader>
              ...
              <CardContent className="p-4 flex flex-wrap items-center gap-3 justify-between">
                <div className="space-y-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <code className="text-sm font-bold tracking-wider">{inv.code}</code>
                    <Badge variant="level">{inv.course_id}</Badge>
                    <Badge variant={inv.status === "open" ? "level" : "secondary"}>
                      {inv.status}
                    </Badge>
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  {inv.status === "open" && (
                    <Button size="sm" variant="ghost" onClick={() => close(inv.id)}>
