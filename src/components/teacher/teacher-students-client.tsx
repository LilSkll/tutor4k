...
export function TeacherStudentsClient() {
  ...
  const [courseId, setCourseId] = React.useState<"all" | "spanish" | "english">("all");
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const q =
        courseId === "all" ? "" : `?courseId=${encodeURIComponent(courseId)}`;
      const res = await fetch(`/api/teacher/students${q}`);
      const data = (await res.json()) as { students?: Row[]; error?: string };
      if (!res.ok) throw new Error(data.error || "fail");
      setRows(data.students ?? []);
    } catch {
      toast.error(t("teacher.students.loadFail"));
    } finally {
      setLoading(false);
    }
  }, [courseId, t]);
  ...
  const revoke = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/teacher/students?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">{t("teacher.students.title")}</h2>
      ...
      <div className="flex flex-wrap gap-2">
        {(["all", "spanish", "english"] as const).map((c) => (
          ...
          >
            {c === "all" ? t("teacher.students.allCourses") : c === "spanish" ? "Español" : "English"}
          </button>
        ))}
      </div>
              ...
              <CardContent className="p-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    ...
                    {student?.level && <Badge variant="level">{student.level}</Badge>}
