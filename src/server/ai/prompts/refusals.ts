...
export function getOffTopicRefusal(
  ...
  // Prefer natural names in Russian UI ("испанский", not bare "Español").
  const nameForUi =
    interfaceLanguage === "ru"
      ? normalizeTargetNameRu(targetLanguageName)
      : targetLanguageName;

  const messages: Record<InterfaceLanguage, string> = {
    ru: `Я помогаю только с изучением ${nameForUi}: грамматика, лексика, произношение, упражнения, а также культура, традиции, литература и поэзия, связанные с этим языком. Вопросы про ChatGPT, нейросети, политику, код, рецепты и другие темы вне языка — не моя роль. Спроси что-нибудь об этом языке!`,
...
function normalizeTargetNameRu(name: string): string {
  const n = name.trim().toLowerCase();
  if (n === "español" || n === "spanish" || n.includes("испан")) {
    return "испанского языка";
  }
  if (n === "english" || n.includes("англи")) {
    return "английского языка";
  }
  if (n === "russian" || n.includes("русск")) {
    return "русского языка";
  }
  return name;
}
